...
"$GOBIN"/client-gen \
  ...
  --output-pkg github.com/sap/image-mapper-cop/pkg/client/clientset \
  --output-dir "$TEMPDIR"/pkg/client/clientset \
  --plural-exceptions ImageMapper:imagemappers

"$GOBIN"/lister-gen \
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-pkg github.com/sap/image-mapper-cop/pkg/client/listers \
  --output-dir "$TEMPDIR"/pkg/client/listers \
  --plural-exceptions ImageMapper:imagemappers \
  github.com/sap/image-mapper-cop/tmp/gen/apis/operator.kyma-project.io/v1alpha1
...
"$GOBIN"/informer-gen \
  ...
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-pkg github.com/sap/image-mapper-cop/pkg/client/informers \
